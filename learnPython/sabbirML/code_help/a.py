...
def write_frame_names_to_files(video_path, base_text_file_path, label, save_directory, target_fps, num_crops):
    ...
    # Calculate frame interval to sample at target FPS
    ...
    for i in range(len(rois)):
        roi_dir = os.path.join(save_directory, f'crop_{i + 1}')
        ...
        create_and_save_text_file(f'{video_name}_person_{i + 1}', roi_dir)
        ...
        if frame_counter % frame_interval == 0:
            ...
            for i, (x, y, w, h) in enumerate(rois):
                ...
                with open(file_path, 'a') as roi_file:
                    roi_file.write(f'{frame_name}  Label={label}  Time={time_str} Person={i + 1}\n')
                    print(f"Added frame name: {frame_name} Label={label} Time={time_str} Person={i + 1}")
...
if __name__ == "__main__":
    ...
    frame_label = input("Enter the default label name for the frames: ")
    output_base_directory = input("Enter the output base directory path: ")
    fps = float(input("Enter the target (FPS) for cropping: "))
    num_crops = int(input("Enter the number of crops per video: "))
